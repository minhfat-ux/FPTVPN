...
#   scripts/ios-watcher.sh once      # chạy 1 lần ngay (không cần bật watcher)
#
...
queue() {
  local t; t=$(ssh "${SSH_OPTS[@]}" "$NODE2" 'systemctl show flowvpn-cp -p Environment | tr " " "\n" | grep "^AUTH_TOKEN=" | cut -d= -f2-' 2>/dev/null || true)
  [ -n "$t" ] || { echo "  (không lấy được AUTH_TOKEN từ node-2 — mạng?)"; return 1; }
  ssh "${SSH_OPTS[@]}" "$NODE2" "curl -s -H 'Authorization: Bearer $t' http://127.0.0.1:7778/v1/admin/ios/devices" | python3 -c '
import json,sys
d=json.load(sys.stdin); pend=[x for x in d["devices"] if not x["built"]]
print(f"  thiết bị: {len(d[\"devices\"])} · chờ ký lại: {len(pend)} · buildSerial: {d[\"buildSerial\"]}")
for x in pend: print("   -", x["udid"], x.get("model"), x.get("registeredAt"))
'
}
...
case "${1:-status}" in
  start)
    ...
    sleep 4
    if pgrep -f "ios-udid-watch.sh" >/dev/null 2>&1; then echo "  ✔ đã bật watcher (PID $(pgrep -f ios-udid-watch.sh | head -1)) · log: $LOG"; queue; else echo "  ✖ không bật được — xem $LOG"; tail -5 "$LOG"; exit 1; fi
    ;;
  stop)
  ...
  once)
    echo "  chạy 1 vòng ngay:"; bash scripts/ios-adhoc-export.sh; ;;
