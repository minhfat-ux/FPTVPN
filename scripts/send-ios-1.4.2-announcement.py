...
Chạy trên node-2:
  send-ios-1.4.2-announcement.py --test              # chỉ gửi tới ALERT_EMAIL
  send-ios-1.4.2-announcement.py                     # gửi cho khách có thiết bị iOS + gói còn hạn
  send-ios-1.4.2-announcement.py --email a@b.com     # gửi 1 địa chỉ
  send-ios-1.4.2-announcement.py --recipients        # chỉ LIỆT KÊ người nhận
...
VI = f"""
  ...
  <p style="background:#eefaf3;border-left:4px solid #0d7a4a;padding:10px 12px;margin:12px 0">
     ...
     Nếu app báo <i>“đã dùng đủ số thiết bị”</i>: vào <b>Cài đặt → Thiết bị</b>, đăng xuất máy cũ rồi bấm Kết nối lại.</p>
  <p>Ngoài ra bản này còn:</p>
  <ul>
    ...
    <li><b>MỚI — tự đăng xuất khỏi thiết bị khác</b> ngay trong app (Cài đặt → Thiết bị), không cần
        nhờ hỗ trợ.</li>
    <li><b>MỚI — watchdog không còn “mù”</b>: bớt hẳn ca app báo “Connected” mà không có mạng.</li>
...
EN = f"""
  ...
  <p><b>One action needed:</b> open the app and <b>sign in again once</b> with your email + OTP code.
     ...
     sign out the old device, then tap Connect again.</p>
  <p>Also in this build:</p>
  <ul>
