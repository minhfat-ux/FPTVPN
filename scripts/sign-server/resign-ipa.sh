...
IPA_LIVE="${IPA_LIVE:-/root/flowvpn-ipa/VPNFlow-latest.ipa}"
...
[ -f "$IPA_LIVE" ] || { echo "LỖI: không thấy IPA đang phát: $IPA_LIVE" >&2; exit 2; }
...
cp "$OUT_IPA" "$IPA_LIVE"; chmod 600 "$IPA_LIVE"
log "     đã thay $IPA_LIVE ($(stat -c %s "$IPA_LIVE") bytes)"
