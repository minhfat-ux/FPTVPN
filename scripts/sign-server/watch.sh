...
CP_HOST="${CP_HOST:-}"
if [ -n "$CP_HOST" ]; then
  PENDING=$(ssh -o BatchMode=yes -o ConnectTimeout=15 "$CP_HOST" 'bash -s' <<'REMOTE' | python3 -c 'import json,sys
...
[ -n "$T" ] && curl -s --max-time 10 -H "Authorization: Bearer $T" http://127.0.0.1:7778/v1/admin/ios/devices
REMOTE
)
else
T=$(systemctl show -p Environment flowvpn-cp | tr " " "\n" | sed -n 's/^AUTH_TOKEN=//p' | head -1)
