...
const GUIDES = {
  ...
  vpn: {
    ...
    zh: {
      title: "网页购买 → 在应用中激活",
      sub: "在购买页用二维码购买 VPNFlow Premium 的激活指南。",
      steps: [
        ["在网页购买", "选择套餐 → 选择支付方式 → 扫码并按正确金额转账（银行二维码和 MoMo 已填好金额）。请记住你填写的邮箱。"],
        ["等待确认到账", "系统核对到账后为该邮箱开通 Premium 并发送发票。通常几分钟内完成。"],
        ["安装应用", "iOS：从 App Store 下载。Android：在购买页下载 APK，并允许“安装未知来源应用”。"],
        ["用购买时的邮箱登录", "打开应用 → 输入**同一个**邮箱 → 点击“发送验证码” → 输入邮件中的验证码，Premium 会自动开启。"],
        ["检查结果", "Logo 旁出现金色皇冠即表示 Premium 已生效。没有？请检查登录邮箱，或带上订单号联系客服。"],
      ],
      faq: [
        ["已付款但应用仍提示需要 Premium？", "请确认登录的邮箱与购买时完全一致。若一致，请等几分钟让系统核对到账，然后重新打开应用。"],
        ["网页购买在 iOS 和 Android 都能用吗？", "可以。Premium 绑定邮箱账号，用同一邮箱登录即可在任意设备使用。"],
        ["网页购买会自动续费吗？", "不会。扫码支付为一次性购买，到期前我们会发邮件提醒。"],
        ["换设备需要重新购买吗？", "不需要。在新设备上用同一邮箱登录即可。"],
      ],
    },
    ja: {
      title: "ウェブで購入 → アプリで有効化",
      sub: "購入ページで QR 決済した VPNFlow Premium の有効化ガイドです。",
      steps: [
        ["ウェブで購入", "プランを選ぶ → 支払い方法を選ぶ → QR を読み取り、正しい金額を送金（銀行 QR と MoMo は金額入力済み）。入力したメールアドレスを控えてください。"],
        ["入金確認を待つ", "入金を照合し、そのメールに Premium を有効化して請求書を送信します。通常数分です。"],
        ["アプリをインストール", "iOS：App Store から。Android：購入ページの APK をダウンロードし、「提供元不明のアプリ」を許可。"],
        ["購入時のメールでログイン", "アプリを開く → **同じ**メールを入力 → 「コードを送信」→ メールのコードを入力すると Premium が自動で有効になります。"],
        ["結果を確認", "ロゴの横に金色の王冠が出れば Premium 有効です。表示されない場合はログイン中のメールを確認し、注文番号を添えてサポートへ。"],
      ],
      faq: [
        ["支払ったのに Premium が必要と表示される", "購入時とまったく同じメールでログインしているか確認してください。同じなら数分待って再度アプリを開いてください。"],
        ["ウェブ購入は iOS と Android の両方で使えますか？", "はい。Premium はメールアカウントに紐づくので、同じメールでログインすればどの端末でも使えます。"],
        ["ウェブ購入は自動更新ですか？", "いいえ。QR 決済は買い切りです。期限前にリマインダーメールをお送りします。"],
        ["機種変更で買い直しが必要ですか？", "不要です。新しい端末で同じメールにログインしてください。"],
      ],
    },
    ko: {
      title: "웹에서 구매 → 앱에서 활성화",
      sub: "구매 페이지에서 QR로 결제한 VPNFlow Premium 활성화 안내입니다.",
      steps: [
        ["웹에서 구매", "요금제 선택 → 결제 수단 선택 → QR을 스캔해 정확한 금액을 이체(은행 QR·MoMo는 금액 입력됨). 입력한 이메일을 기억하세요."],
        ["입금 확인 대기", "입금을 대조해 해당 이메일에 Premium을 활성화하고 청구서를 보냅니다. 보통 몇 분 걸립니다."],
        ["앱 설치", "iOS: App Store에서 설치. Android: 구매 페이지에서 APK를 받고 ‘알 수 없는 출처’ 설치를 허용."],
        ["구매한 이메일로 로그인", "앱 실행 → **동일한** 이메일 입력 → ‘코드 보내기’ → 메일의 코드 입력 → Premium이 자동으로 켜집니다."],
        ["결과 확인", "로고 옆에 금색 왕관이 보이면 Premium 활성 상태입니다. 안 보이면 로그인한 이메일을 확인하거나 주문번호와 함께 지원팀에 문의하세요."],
      ],
      faq: [
        ["결제했는데 앱이 계속 Premium을 요구합니다", "구매할 때와 정확히 같은 이메일로 로그인했는지 확인하세요. 맞다면 몇 분 기다린 뒤 앱을 다시 열어 주세요."],
        ["웹 구매가 iOS와 Android 모두에서 되나요?", "됩니다. Premium은 이메일 계정에 연결되므로 같은 이메일로 로그인하면 어느 기기에서도 사용할 수 있습니다."],
        ["웹 구매가 자동 갱신되나요?", "아니요. QR 결제는 1회 구매이며 만료 전에 알림 메일을 보내드립니다."],
        ["기기를 바꾸면 다시 사야 하나요?", "아니요. 새 기기에서 같은 이메일로 로그인하면 됩니다."],
      ],
    },
