# Triển khai FlowGpt lên node-2

Máy đích: **node-2** (`fcnvps2`) — `165.101.114.162`, nơi Caddy đang phục vụ `meetflowai.site` + `t1.meetflowai.site`
và control-plane `flowvpn-cp` (port 7778). FlowGpt chạy như một service riêng ở **127.0.0.1:7790**, Caddy làm TLS.
...
| **Bản ghi DNS `flowgpt.meetflowai.site`** | ❌ **cần tạo — xem bước 2** |
...
→ `scp` lên `/tmp` → giải nén vào `/opt/flowgpt` + `npm install` cho server (bỏ devDeps) → chạy
`deploy/remote-setup.sh` → kiểm tra `https://flowgpt.meetflowai.site/api/health`.
...
|---|---|---|---|
| A | `flowgpt` | `165.101.114.162` | Proxied (cam) |
...
bash /opt/flowgpt/deploy/dns-cloudflare.sh            # xem trước (dry-run)
bash /opt/flowgpt/deploy/dns-cloudflare.sh --apply    # ghi thật + chờ HTTP 200
```
...
1. Tạo `/opt/flowgpt`, `/var/lib/flowgpt`, `/etc/flowgpt` (700).
2. Sinh `/etc/flowgpt/flowgpt.env` (600) với `FLOWGPT_SECRET` random **một lần** — các lần deploy sau giữ nguyên.
3. Cài `/etc/systemd/system/flowgpt.service`, `daemon-reload`, `enable`.
4. Thêm block Caddy cho `flowgpt.meetflowai.site` → `reverse_proxy 127.0.0.1:7790`
   (**backup `.bak-flowgpt-<timestamp>` → `caddy validate` → `reload`; validate fail thì tự khôi phục**).
5. `systemctl restart flowgpt`, chờ service lên, `curl` `/api/health`.

Chạy lại thủ công khi cần:

```bash
ssh root@165.101.114.162
journalctl -u flowgpt -f            # log
systemctl restart flowgpt           # khởi động lại
...
```bash
curl -s https://flowgpt.meetflowai.site/api/health   # {"ok":true,...}
curl -sI https://flowgpt.meetflowai.site/            # 200 + text/html
```
...
Dữ liệu nằm ở `/var/lib/flowgpt` (SQLite + tệp + artifact) **không** bị ghi đè khi deploy.
Backup: `cp -a /var/lib/flowgpt /root/flowgpt-backup-$(date +%F)`.

## 6. Rollback

```bash
# quay lại Caddyfile trước khi thêm FlowGpt
cp -a /etc/caddy/Caddyfile.bak-flowgpt-<timestamp> /etc/caddy/Caddyfile
caddy validate --config /etc/caddy/Caddyfile && systemctl reload caddy
systemctl disable --now flowgpt        # tắt app (dữ liệu vẫn còn ở /var/lib/flowgpt)
```

## 7. Biến môi trường (`/etc/flowgpt/flowgpt.env`, 600)

| Biến | Ý nghĩa |
|---|---|
| `FLOWGPT_SECRET` | Ký JWT + mã hoá AES-256-GCM cho API key provider/MCP. **Đổi = mọi key đã lưu không đọc được.** |
| `FLOWGPT_PUBLIC_URL` | `https://flowgpt.meetflowai.site` (CORS + cookie `Secure`) |
| `FLOWGPT_HOST` / `FLOWGPT_PORT` | `127.0.0.1` / `7790` |
| `FLOWGPT_DATA_DIR` | `/var/lib/flowgpt` |
| `FLOWGPT_RESEND_API_KEY` | (tuỳ chọn) gửi mã đăng nhập; có thể dán trong UI Cài đặt thay vì để đây |
