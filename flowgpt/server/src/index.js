...
app.use((err, _req, res, _next) => {
  ...
  if (status >= 500) console.error("[flowgpt]", err);
  if (res.headersSent) return res.end();
...
const server = app.listen(config.port, config.host, async () => {
  ...
  console.log(
    `[flowgpt] ${config.appName} v${config.version} listening on http://${config.host}:${config.port} ` +
      `(env=${config.env}, data=${config.dataDir}, web=${hasWeb ? "built" : "dev-proxy"}, users=${countUsers()})`,
  );
  if (!countUsers()) {
    console.log("[flowgpt] Chưa có tài khoản nào — người đăng ký ĐẦU TIÊN sẽ là quản trị viên.");
  }
  if (!settings.defaultProviderId) {
    console.log("[flowgpt] Chưa cấu hình nhà cung cấp AI nào — thêm trong Cài đặt sau khi đăng nhập.");
  }
  try {
    const results = await connectAll();
    for (const result of results) {
      console.log(`[flowgpt] MCP ${result.id}: ${result.ok ? `${result.toolCount} tool` : `lỗi — ${result.error}`}`);
    }
  } catch (err) {
    console.warn("[flowgpt] MCP bootstrap lỗi:", err?.message ?? err);
  }
});

for (const signal of ["SIGINT", "SIGTERM"]) {
  process.on(signal, async () => {
    console.log(`[flowgpt] ${signal} — đang dừng…`);
    server.close();
