...
# FlowGpt's env file, restart, and reset the data so the owner's email becomes
...
grep -v '^FLOWGPT_RESEND_API_KEY=' "$ENV_FILE" > "$ENV_FILE.tmp" 2>/dev/null || true
printf 'FLOWGPT_RESEND_API_KEY=%s\n' "$KEY" >> "$ENV_FILE.tmp"
install -m 600 "$ENV_FILE.tmp" "$ENV_FILE"
rm -f "$ENV_FILE.tmp"
echo "Đã ghi FLOWGPT_RESEND_API_KEY vào $ENV_FILE (mode $(stat -c %a "$ENV_FILE"))"
