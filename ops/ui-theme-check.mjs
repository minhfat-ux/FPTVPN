...
const [url = "https://fbuddy.meetflowai.site", outDir = "ops/ui-out-theme", portArg, tokenArg] = process.argv.slice(2);
...
await sleep(3000);
await evaluate(`localStorage.setItem("fbuddy.token", ${JSON.stringify(token)}); localStorage.setItem("fbuddy.locale","vi"); localStorage.removeItem("fbuddy.ecosystem.banner"); localStorage.removeItem("fbuddy:promo:apps:v1");`);
await send("Page.navigate", { url });
