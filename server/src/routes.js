...
export function createApiRouter() {
  ...
  router.patch(
    ...
    asyncHandler(async (req, res) => {
      ...
      const updated = updateProfile(req.user.id, { name: req.body?.name, locale: req.body?.locale });
  ...
  router.get("/hub", requireAuth, (req, res) => {
    const items = listHubSkills({ userId: req.user.id, lang: requestLang(req) });
    res.json({
      items,
  ...
  router.get("/hub/:id", requireAuth, (req, res) => {
    const skill = getHubSkill(req.params.id, { userId: req.user.id, lang: requestLang(req) });
  ...
  router.get("/admin/hub", requireAdmin, (req, res) => {
    res.json({
