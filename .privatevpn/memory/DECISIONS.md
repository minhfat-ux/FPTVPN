...
| 2026-08-21 | **macOS target `PrivateVPNMac`** added; reuses ControlAPIClient; join token auto-fetched from `POST /v1/tokens` when empty. The initial `wg-quick` local test path is superseded by NetworkExtension + WireGuardKit. | project.yml, mac/PrivateVPNMac/ |
