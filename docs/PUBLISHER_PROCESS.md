...
| 15 | **KHÓA KÊNH khi chủ dự án đang cho khách test** (26/09/2026): khóa ghi ở `.privatevpn/status/release-lock.json`; cổng `scripts/check-publish-version.py` **CHẶN cứng (exit 3)** mọi lần định phát bản KHÁC bản đang khóa (vẫn cho kiểm lại đúng bản đang khóa). Mở khóa = sửa file đó (có chủ đích) hoặc `--allow-locked "<lý do>"` (lý do được in ra). Ca đầu: **macOS 1.4.7/28** — chủ dự án *“để khách test đã”* | §0 luật này · `.privatevpn/status/release-lock.json` | mọi publisher |
