...
| 2026-09-26 | macOS | **1.4.7 (28)** | 🔒 **LOCKED — chốt của chủ dự án (26/09/2026): *“bản macOS tạm thời dùng OK rồi, để khách test đã”* ⇒ KHÓA kênh macOS ở đúng 1.4.7/28, KHÔNG phát bản macOS mới cho tới khi hết kỳ khách test.** Kênh đang phục vụ ĐÚNG bản này, đo lại lúc khóa: `GET /v1/app-version?platform=macos` ⇒ `minimum_version=1.4.7`, `latest_version=1.4.7`; tải thật qua `t1` ⇒ **23.798.393 B · sha256 `d83646503476f73bfca2e6a5b06b641f1cbdca534127368e353703174b3d2346`** (khớp dòng sổ `1.4.7/28` + tag `macos-v1.4.7`); `project.yml` macOS `1.4.7/28`; máy thật `systemextensionsctl list` ⇒ `com.privatevpn.mac.packet-tunnel (1.4.7/28) [activated enabled]`. **Việc bị khóa:** mọi build/`mac-sign-notarize.sh`/upload DMG mới cho kênh macOS; nếu cần sửa thì làm ở **build 29** và chỉ phát sau khi chủ dự án cho phép. **CÒN NỢ giữ nguyên (không phải điều kiện mở khóa):** (1) chưa test cài MỚI hoàn toàn trên máy sạch (bước khách bấm Allow cho system extension) — cần 1 máy/user mới; (2) biên failover 26 s so với mục tiêu <30 s; **(3) MỚI 26/09 19:00–19:09 (+08) — `BUG-MACOS-SESSION-DROP-001` (mức `high`, đã ghi vào `.privatevpn/status/bugs.json`):** bật/tắt Tailscale làm tunnel rơi 2 lần, lần 2 app **KHÔNG tự nối lại** (`scutil --nc` Disconnected, mất `utun` + route, egress đi thẳng ISP `120.234.32.53`) — nếu khách gặp lại trong kỳ test này thì đây là lý do mở build 29; bằng chứng `.privatevpn/reports/2026-09-26-mac-tunnel-drop-after-tailscale-toggle.md` |
