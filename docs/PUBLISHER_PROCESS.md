...
| Keychain group iOS (CHỮ KÝ) | `codesign -d --entitlements - /tmp/x/Payload/*.app` **và** `…/PlugIns/*.appex` | chữ ký phải khai `G6XW3RN6LJ.com.privatevpn.shared`; app **và** extension **cùng một nhóm** |
| Keychain group iOS (PROFILE) | `security cms -D -i /tmp/x/Payload/*.app/embedded.mobileprovision` (và profile của `.appex`) → `Entitlements.keychain-access-groups` | profile phải **liệt kê ĐÚNG** nhóm trên — wildcard `G6XW3RN6LJ.*` KHÔNG đủ. 22/09/2026: profile chỉ có wildcard ⇒ `SecItemAdd` trả `errSecMissingEntitlement` (-34018) ⇒ app kẹt màn hình đăng nhập. Sinh profile chứa nhóm: Apple Developer portal → Identifiers → App ID `com.privatevpn.app` **và** `com.privatevpn.app.packet-tunnel` → bật **Keychain Sharing** → thêm nhóm `com.privatevpn.shared` → regenerate profile Ad Hoc |
| application-identifier iOS | `codesign -d --entitlements - <bundle>` vs `embedded.mobileprovision` | khớp nhau (appex 1.4.0/16 bị ký bằng entitlements của app ⇒ chữ ký `.app` ≠ profile `.packet-tunnel`) |
...
| Cổng chặn version §2b (chạy TRƯỚC khi upload) | `python3 scripts/check-publish-version.py --platform <ios\|android\|android-legacy\|macos\|windows> --file <artifact> --version <ver> [--build <n>]` | exit **0 = ĐẠT** · **1 = KHÔNG ĐẠT ⇒ DỪNG** · **2 = không kiểm được ⇒ chạy lại trên máy đủ công cụ** (macOS cho DMG, Windows cho `.exe`, Android SDK cho APK). Sau khi upload kiểm lại bằng `--mode post`. **Với iOS**, cổng còn kiểm **nhóm keychain trong CHỮ KÝ vs PROFILE** (app + extension, `--require-keychain-group`, mặc định `G6XW3RN6LJ.com.privatevpn.shared`) và `application-identifier` — thiếu là DỪNG; mặc định KHÔNG chấp nhận wildcard `TEAMID.*` (nới bằng `--allow-keychain-wildcard`, chỉ khi Apple xác nhận) |
...
| 2026-09-20 | macOS | **1.4.0 (14)** | **Ký Developer ID + notarize** (DMG 21.617.309 B) → khách mở không cảnh báo · email 3 ngôn ngữ gửi 17/17 khách. ⚠️ **Sửa 22/09/2026 (đọc thẳng artifact): bản ghi cũ ghi "1.3.3 (13) + staple" là SAI** — `Info.plist` trong app + extension đang phát là 1.4.0/14, và `stapler validate` báo **CHƯA staple** (tuy `spctl` vẫn `Notarized Developer ID`). |
