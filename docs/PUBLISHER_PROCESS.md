...
| 2026-09-26 | iOS | **1.4.6 (54) — THAY bản 50** | **NGOẠI LỆ CÓ GHI SỔ của chủ dự án (chốt trong phiên 26/09, lúc 03:4x):** cho phép phát **cùng số version 1.4.6 nhưng KHÁC sha256** (`363fd7a8…`) để khách TQ nhận ngay bản chống *"bấm Connect là fail"* trên 5G — nội dung: `relayOpenGrace` 6 → **10 s**, thêm **cửa vào thứ hai `t1.meetflowai.site`** vào `relayURLCandidates`, ngân sách phiên 20 → **35 s**, gỡ `ipv6Settings` (lần 2). Đây là ngoại lệ **có ý thức** với `docs/VERSIONING.md` §3.3 (artifact bất biến) — bản 50 **giữ nguyên dòng sổ riêng**, không xoá; bản 54 ghi dòng mới. Cổng: `ios-verify-ipa.sh` (1.4.6/54, credential 21+12, ad-hoc **10 UDID**, `get-task-allow=False`) + `check-publish-version.py` pre **ĐẠT** (cảnh báo "phát lại cùng số" đã được chốt này gỡ). §2c: `build/ios-146-device-test-54.md` — mục 1/2/4/6/7 ĐẠT, **mục 3 chỉ một phần** (phiên iPad 63 s), **mục 5 chưa test lại cho 54**; từ build 53 log chi tiết **mặc định TẮT** nên cổng chấm log bỏ qua phiên thiếu dòng (đã bật `Documents/tunnel-log-on` trên cả 2 máy lúc 03:35 để lần sau chấm được) |
