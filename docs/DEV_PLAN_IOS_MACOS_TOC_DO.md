...
|---|---|---|---|
| **A7** | **App TQ đi ĐƯỜNG RIÊNG, không qua VPN** (§2d): WeChat/Alipay/Meituan/Didi/Taobao… đăng nhập + giữ kết nối khi VPN bật; **byte của tunnel KHÔNG tăng** khi chỉ dùng app TQ | **ĐÃ CHỐT hướng (a) 22/09 (chủ dự án) + Mac đã code xong phía client (`ebb45df`: `ChinaRouteBypass.swift` + `excludedRoutes`).** Sự thật đã soát code:<br>• **Dữ liệu ĐÃ CÓ và đang phục vụ**: `https://meetflowai.site/dl/routes/cn.txt` — **5.494 prefix IPv4** (85.375 byte), sinh từ **APNIC delegation**; đã đối chiếu: `scripts/gen-cn-cidrs.mjs` sinh ra **trùng khít** file đang phát (dùng để refresh định kỳ). ⚠️ **IPv6 — chủ dự án đã chốt 22/09: phát thêm và cho CẢ 3 nền tảng dùng.** Sự thật đã đo: **server KHÔNG có IPv6** (`curl -6` fail, không default route, `forwarding=0`) ⇒ **không** đưa `::/0` vào tunnel (sẽ đen hết IPv6); thiết kế đúng là **IPv6 của TQ đi thẳng, IPv6 còn lại CHẶN** (không rò IP thật). Dữ liệu IPv6: `docs/routes/cn6.txt` = **2.015 prefix** (29,6 KB) sinh từ cùng nguồn APNIC, đã kiểm: phủ đủ 2.043 dải APNIC, không prefix bịa. Chia việc: **Windows** thêm route IPv6 bypass cho dải TQ (đang chặn sạch `::/1 + 8000::/1`); **iOS/macOS** đẩy `cn6.txt` vào `NEIPv6Route` excludedRoutes — ⚠️ **kèm lỗi bảo mật đang có**: provider chỉ cấu hình `ipv4`, **không có `ipv6Settings`** ⇒ hiện IPv6 đi thẳng ra ngoài cho MỌI đích (rò IP thật).<br>• **Windows ĐÃ LÀM A7 từ trước** (khác mô tả §2d): `ChinaBypass.cs` tải `cn.txt` rồi `WintunWireGuardDriver` **thêm route bypass qua interface vật lý** — không dùng rule `geosite/geoip`. Rule CN cho **đường relay sing-box vẫn là TODO** (`SingBoxConfigBuilder.cs:43`).<br>• **iOS/macOS**: extension là **hysteria-only** (`project.yml`) ⇒ **KHÔNG có** `geosite:cn`/`geoip:cn`; dùng `NEPacketTunnelNetworkSettings.excludedRoutes` (`includedRoutes` = `0.0.0.0/0`) — Mac đã làm. **Nên bundle sẵn `docs/routes/cn.txt` + `cn6.txt` trong app** để A7 chạy được ngay lần đầu/dưới mạng yếu, cập nhật ở luồng nền, **không** chặn kết nối (bài học Windows 1.0.4 "connecting mãi"). ⚠️ **5.494 + 2.015 route là nhiều** — phải đo thời gian connect trên máy thật.<br>• Đo kiểm: WeChat/Alipay đăng nhập + giữ kết nối **cả trên IPv4 lẫn IPv6**, **byte tunnel không tăng** khi chỉ dùng app TQ | Mac (iOS/macOS) · Windows (đã xong IPv4, đang thêm IPv6) |
