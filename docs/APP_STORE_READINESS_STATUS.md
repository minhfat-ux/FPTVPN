...
| # | Việc | Ai làm | Ghi chú |
|---|---|---|---|
| 1 | Đồng bộ StoreKit ↔ backend: verify giao dịch rồi cấp gói | Dev | **Blocker chức năng**: hiện server không biết giao dịch StoreKit nên khách mua thật bị `403 Active subscription required` |
...
| 4 | IAP trên ASC: `Monthly_Premium`, `Yearly_Premium` ở trạng thái *Ready to Submit*, giá khớp web | Chủ dự án | Thiếu ⇒ paywall rỗng, dễ bị từ chối 2.1 |
...
| 12 | macOS: version 1.2.2 lệch iOS, thiếu `Mac_monthly`/`Mac_yearly`, profile NE chưa có, và `VPNManagerMac.swift:430-435` **nhét cả private key** vào `providerConfiguration` (iOS đã strip) | Dev | Chỉ cần khi nộp kèm macOS |
