...
final class MacSubscriptionStore: ObservableObject {
    ...
    /// Tên gói hiển thị ở menu bar + thẻ Subscription: backend gửi `plan_badge`
    /// ("Monthly" / "3 Months" / "Yearly"…); payload cũ không có thì suy từ `product_id`.
    ...
    /// Ngày hết hạn của gói đã mua (nil = vĩnh viễn hoặc chưa mua).
